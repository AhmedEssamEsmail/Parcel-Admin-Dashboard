-- Enhancements v2: city SLA configuration table
...
create index if not exists idx_city_sla_configs_lookup
  on public.warehouse_city_sla_configs (warehouse_id, city_normalized);

comment on table public.warehouse_city_sla_configs is
  'City-level SLA overrides per warehouse. city_normalized should be trimmed/lowercased.';
...
drop view if exists public.v_parcel_kpi cascade;

create view public.v_parcel_kpi as
with eff as (
  select
    b.*,
