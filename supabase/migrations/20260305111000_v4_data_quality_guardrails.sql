...
create or replace view public.v_raw_delivery_stages_with_source as
select
  rds.*,
  case
    when dtr.city_normalized is not null then 'CITY_RULE'
    else 'WAREHOUSE_FALLBACK'
  end as timing_source
from public.v_raw_delivery_stages rds
join public.warehouses w
  on w.code = rds.warehouse
left join public.warehouse_delivery_timing_rules dtr
  on dtr.warehouse_id = w.id
 and dtr.city_normalized = lower(btrim(coalesce(rds.city, '')));
