        ...
        <tbody>
          {rows.map((row, idx) => (
            <tr key={`${row.day}-${row.warehouse_code}-${row.city}-${idx}`}>
              <td>{row.day}</td><td>{row.warehouse_code}</td><td>{row.city}</td><td>{row.total_orders}</td><td>{row.delivered_with_promise}</td><td>{row.within_promise_window}</td><td>{row.promise_hit_rate?.toFixed(2) ?? "-"}</td><td>{row.avg_eta_error_minutes?.toFixed(2) ?? "-"}</td>
            </tr>
          ))}
        </tbody>
      </table>
